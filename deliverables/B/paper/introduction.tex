...
\liam{Swap clauses in this sentence: In contrast....which operate, Cogent is suitable for low level...., as its uniqueness...}Cogent is also suitable for low level systems development in contrast to many existing functional languages which operate
...
Currently, any data type that can be iterated over and its iterators have to be defined externally in C,
  and included in the Cogent program via \liam{no its or no Cogent's}its Cogent's C FFI.
...
However\liam{put however between commas after exacerbated? Strunk and White recommend not starting with However} the cost of reasoning about a particular program's termination is
  exacerbated \liam{by}with the overhead of handwritten C code, due to iteration being an external construct to the language.
...
While providing this benefit we must also respect the existing guarantees that Cogent enjoys,\liam{not all of these particulars are guarantees}
