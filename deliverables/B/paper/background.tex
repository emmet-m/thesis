...
\liam{Doesn't have to be the same. Not sure what new stuff you have though?}
...
\liam{If you want to get the alignment right in the figures, just use an 
array environment inside a math environment.}
%%% Basic syntax
\begin{figure}
    \centering
...
system could cause it to hang, denying services to other parts of a system which could be core to \liam{the?}
system's function.

To deal with termination, we must consider the environment where we will prove termination for
Cogent programs, \liam{emdashes, not commas} the Isabelle embedding, and how we can make this process easier on the type
level within Cogent.
...
an explicit termination proof (which is required to reason inductively about any Isabelle function.) \liam{something's weird punctuationally here}
...
In his tutorial, Krauss~\citep{KraussIsabelle}\liam{no date?} discusses the details of the latter two of the three methods
...
terminate or because Isabelle cannot prove it does alone\liam{no alone}). In contrast to \textbf{fun}, \textbf{function}
requires that the termination proofs be solved\liam{provided?} manually by whoever is writing the function.
...
\theoremstyle{definition}
\begin{definition}
    A data type $T$ is a \textit{positive} data type if \liam{it?} only appears nested in a positive position
...
This is not an issue in Haskell due to its lazy evaluation and \liam{"permissiveness of general recursion" rather than "Haskell allows..freely"} Haskell allows its programmers to write
...
situation will hang when \textit{infiniteExpression} is constructed\liam{evaluated?}.
...
eliminate them from our language \liam{It doesn't matter if it's accidental or not}.

Many theorem provers and dependently typed languages make use of \textit{strictly positive} types, which
prohibit the construction of infinitely recursive data structures, that under a dependant\liam{dependent} type system
both negative and simple positive types allow.\liam{Didn't explain how non-strictly positive types allow this?}
...
or non-strictly positive types introduce logical inconsistencies which can be used to prove false statements,\liam{That's what logical inconsistencies are, don't need to define them}
something that is unacceptable for a theorem prover.

The definition of strictly positive is \liam{given by Coquand and Paulin as follows:}discussed by \citet{CoquandTypes}, and is as follows:
...
That is, to prove any predicate $P$ inductively over nats\liam{nats?} ($\forall(N : Nat).\; P(N)$)
...
\liam{I would describe the type of an eliminator on nats too, to show the similarity to the induction principle}

The interactive theorem prover Isabelle generates the same induction principle for any type created in
Isabelle\liam{As opposed to types created outside Isabelle?}. We can get the same induction principle over natural numbers by redefining our $Nat$
...
by the use once rule\liam{That's not what happens. It just means that there is only ever one pointer to a particular allocation, so if you allow the pointer to go out of scope without using it you know it has been leaked}. The result is a language that does not require a garbage collector,
can check that programs appropriately handle allocated resources statically as this can now
be determined statically. \liam{This is called uniqueness types.}
...
and thus destructive updates on these variables are safe. \liam{That's not totally accurate. His "Change the World" system is also a system of uniqueness types.}
