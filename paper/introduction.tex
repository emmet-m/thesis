...
One of the benefits of doing so is a proof of correctness for programs with respect to a specification,
  and\liam{which} for well specified programs aids in eliminating bugs and unexpected program behaviour. \liam{Can afford to be a bit stronger. Use the word ``eliminate'' wrt bugs}
...
Proving totality about existing code is desirable as proofs for the correctness of programs only hold
  if they terminate\liam{Not true. Totality/termination is just a prerequisite for correctness of most systems. But non-terminating systems (e.g. corecursive ones) can be correct too.},
...
This thesis aims to introduce recursive types to Cogent's type system, allowing internal iteration over
  internal data structures without the involvement of \liam{handwritten} C code.
While providing this benefit we must also respect the existing guarantees that Cogent enjoys,
  in particular, simple reasoning about functional correctness and \liam{, not and} totality,
  its static memory allocation\liam{,} and destructive updates\liam{,} all while keeping in mind
  a possible efficient C representation for the later implementation of Cogent to C\liam{``Cogent to C'' isn't a noun. Talk about compilation}.
